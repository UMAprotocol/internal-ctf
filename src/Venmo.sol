...
// Challenge:
// This contract is a simple venmo contract where you can deposit money and send it to others.
// All operations can be done via direct calls or gaslessly via signatures.
// Your goal is to steal money.
...
contract Venmo {
    ...
    function depositBySig(
        ...
        bytes32 hashedInfo = keccak256(
            abi.encode("Venmo::depositBySig", amount)
        );
    ...
    function withdrawBySig(
        ...
        bytes32 hashedInfo = keccak256(
            abi.encode("Venmo::withdrawBySig", amount)
        );
        require(ECDSA.recover(hashedInfo, signature) == user);
    ...
    function sendBySig(
        ...
        bytes32 hashedInfo = keccak256(
            abi.encode("Venmo::sendBySig", to, amount)
        );
